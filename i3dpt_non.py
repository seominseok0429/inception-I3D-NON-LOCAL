...
class I3D_NON(torch.nn.Module):
    def __init__(self,
                 num_classes=400,
                 modality='rgb',
                 dropout_prob=0,
                 name='inception'):
        super(I3D_NON, self).__init__()
        ...
        self.NonLocalBlock_mixed_3c = NonLocalBlock(480,480,240)    # Non-local
        ...
        self.NonLocalBlock_mixed_4d = NonLocalBlock(512,512,256)    # Non-local
    ...
    def forward(self, inp):
        ...
        out = self.NonLocalBlock_mixed_3c(out)
        out = self.maxPool3d_4a_3x3(out)
        out = self.mixed_4b(out)
        out = self.mixed_4c(out)
        out = self.mixed_4d(out)
        out = self.NonLocalBlock_mixed_4d(out)
...
class NonLocalBlock(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels):
        ...
        self.theta = nn.Conv3d(in_channels, hidden_channels, kernel_size=(1,1,1), stride=(1,1,1), padding=(0,0,0))
        self.maxpool = nn.MaxPool3d(kernel_size=(1,2,2), stride=(1,2,2), padding=(0,0,0))
        self.phi = nn.Conv3d(in_channels, hidden_channels, kernel_size=(1,1,1), stride=(1,1,1), padding=(0,0,0))
        self.g = nn.Conv3d(in_channels, hidden_channels, kernel_size=(1,1,1), stride=(1,1,1), padding=(0,0,0))

        self.out = nn.Conv3d(hidden_channels, out_channels, kernel_size=(1,1,1), stride=(1,1,1), padding=(0,0,0))
    ...
    def forward(self, x):
        ...
        theta = self.theta(x)
        phi = self.phi(maxpool)
        g = self.g(maxpool)

        theta_shape_5d = theta.shape
        theta, phi, g = theta.view(batch_size, self.hidden_channels, -1), phi.view(batch_size, self.hidden_channels, -1), g.view(batch_size, self.hidden_channels, -1)
      
        theta_phi = torch.bmm(theta.transpose(1, 2), phi) # batch mat mul
        theta_phi_scale = theta_phi * (self.hidden_channels**-.5) # https://github.com/facebookresearch/video-nonlocal-net/issues/65
        p = F.softmax(theta_phi_scale, dim=-1)
